...
export class CajasService {
  ...
  async cerrarCaja(cajaId: string): Promise<Caja> {
    ...
    const totalVentasQR = ventas.filter(v => v.tipo_pago === 'QR').reduce((sum, venta) => sum + venta.total, 0);
    const totalVentasEfectivo = ventas.filter(v => v.tipo_pago === 'EFECTIVO').reduce((sum, venta) => sum + venta.total, 0);
  ...
  async findOne(id: string): Promise<Caja> {
    const caja = await this.cajaRepository.findOne({
      where: { id },
      relations: ['usuario', 'ventas', 'gastos', 'gastos.usuario', 'gastos.categoria', 'ventas.vendedor', 'ventas.cliente'],
    });
